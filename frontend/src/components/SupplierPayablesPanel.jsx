...
export function SupplierPayablesPanel({ startDate, endDate, onChanged }) {
	...
	const purchaseTotal = (parseInt(purchaseQuantity || "0", 10) * parseFloat(purchaseCost.replace(/\./g, "").replace(",", ".") || "0")).toFixed(2);
	...
	return (
		<div className="space-y-3 px-4 pb-6 pt-4">
			...
			{showPurchaseForm && (
				<div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/70" onClick={() => setShowPurchaseForm(false)}>
					<form onSubmit={handlePurchase} className="max-h-[90dvh] overflow-y-auto w-full max-w-[520px] rounded-t-lg border-x border-t border-border bg-background p-4 pb-6" onClick={(event) => event.stopPropagation()}>
						<div className="flex items-center justify-between">
							<p className="font-mono-ui text-[10px] uppercase text-foreground-faint">Entrada de Estoque</p>
							<button type="button" onClick={() => setShowPurchaseForm(false)} className="text-xl leading-none text-foreground-faint">&times;</button>
						</div>
						<h2 className="mt-1 font-logo text-lg text-foreground">Nova Compra</h2>
						
						<div className="mt-4 space-y-3">
							<label className="block">
								<span className="mb-1 block font-mono-ui text-[10px] text-foreground-faint">Produto do catálogo</span>
								<select 
									required 
							...
							<label className="block">
								<span className="mb-1 block font-mono-ui text-[10px] text-foreground-faint">Fornecedor</span>
								<input required type="text" value={purchaseSupplier} onChange={(e) => setPurchaseSupplier(e.target.value)} placeholder="Nome do fornecedor" className="w-full rounded-md border border-border bg-secondary px-3 py-3 text-sm text-foreground" />
							</label>
							
							<div className="grid grid-cols-2 gap-3">
								<label className="block">
									<span className="mb-1 block font-mono-ui text-[10px] text-foreground-faint">Quantidade comprada</span>
									<input required type="number" min="1" step="1" value={purchaseQuantity} onChange={(e) => setPurchaseQuantity(e.target.value)} className="w-full rounded-md border border-border bg-secondary px-3 py-3 text-sm text-foreground" />
								</label>
								<label className="block">
									<span className="mb-1 block font-mono-ui text-[10px] text-foreground-faint">Custo unitário (R$)</span>
									<input required type="text" inputMode="decimal" placeholder="0,00" value={purchaseCost} onChange={(e) => setPurchaseCost(e.target.value)} className="w-full rounded-md border border-border bg-secondary px-3 py-3 text-sm text-foreground" />
								</label>
							</div>
							
							<div className="rounded-md bg-card p-3 border border-border">
								<div className="flex justify-between items-center mb-3">
									<span className="font-mono-ui text-[10px] text-foreground-faint">VALOR TOTAL:</span>
									<span className="font-value text-lg text-foreground">R$ {purchaseTotal}</span>
								</div>
								
								<label className="block">
									<span className="mb-2 block font-mono-ui text-[10px] text-foreground-faint">Situação do pagamento</span>
						...
						<button type="submit" disabled={isSaving || !purchaseProduct || !purchaseSupplier} className="mt-4 w-full rounded-md bg-foreground py-3 font-mono-ui text-sm text-primary-foreground disabled:opacity-60">
