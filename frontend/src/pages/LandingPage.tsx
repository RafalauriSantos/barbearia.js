...
export default function LandingPage() {
	...
	return (
		<main className="h-[var(--app-height)] overflow-y-auto overflow-x-hidden bg-background-deep text-white">
			<section className="relative min-h-[88svh] overflow-hidden">
				...
				<div className="absolute inset-x-0 top-0 z-20">
					<nav
						...
						<Link
							...
							<span
								aria-hidden="true"
								className="relative flex h-10 w-10 items-center justify-center rounded-md border border-primary/35 bg-primary/10">
								<span className="absolute h-[3px] w-5 rotate-[-34deg] rounded-full bg-primary" />
								<span className="absolute mt-1 h-[3px] w-4 rounded-full bg-status-fiado" />
							</span>
							<span className="min-w-0">
								<BrandName size="sm" className="block text-white" />
								<span className="block truncate font-client text-xs text-primary/75">
						...
						<Link
							to="/login"
							className={`hidden rounded-md border border-white/[0.16] bg-white/[0.08] px-4 py-2 font-client text-xs text-white/82 backdrop-blur transition-[background-color,border-color,color] hover:border-status-fiado/45 hover:bg-white/[0.12] hover:text-white sm:inline-flex ${focusClass}`}>
							Entrar
						</Link>
					</nav>
				</div>
				...
				<div className="relative z-10 mx-auto flex min-h-[88svh] w-full max-w-6xl items-center px-5 pb-16 pt-28 sm:px-7 lg:pt-32">
					<div className="max-w-[690px]">
						<p className="font-client text-xs font-semibold text-primary tracking-wide">
						...
						<div className="mt-9 grid w-full max-w-xl grid-cols-3 gap-2 border-y border-white/[0.12] py-4">
							{metrics.map(([value, label]) => (
								<div key={label} className="min-w-0">
									...
									<p className="mt-1 break-words font-client text-[10px] text-white/[0.54]">
										{label}
									</p>
								</div>
							))}
			...
			<section className="border-t border-white/10 bg-background-deep px-5 py-12 sm:px-7 lg:py-16">
			...
			<section className="bg-background px-5 py-12 sm:px-7 lg:py-16">
				<div className="mx-auto grid w-full max-w-6xl gap-8 lg:grid-cols-[1.04fr_0.96fr] lg:items-center">
					<div className="rounded-md border border-white/10 bg-card p-4 shadow-2xl shadow-black/25">
						<div className="flex items-center justify-between border-b border-white/10 pb-4">
							<div className="min-w-0">
								<p className="font-client text-xs text-white/50">
									Hoje
								</p>
							...
							<p className="font-value text-3xl text-primary">R$ 420</p>
						</div>
								...
								<div
									...
									<span
										className={`rounded-md border px-2 py-1 font-mono-ui text-[9px] ${
											...
											: status === "Fiado" ?
												"border-status-fiado/25 bg-status-fiado/10 text-status-fiado"
					...
					<div>
						...
						<div className="mt-6 grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
							{highlights.map(([title, text]) => (
								<div
									...
									<p className="font-client text-xs font-bold text-status-fiado">
										{title}
									</p>
						...
						<PrimaryLink
							to="/login?mode=signup"
							className="mt-7 bg-white text-background-deep hover:bg-status-fiado">
							Começar agora
						</PrimaryLink>
					</div>
				</div>
